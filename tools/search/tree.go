...
func (r *Result) GetRoute() string {
	if len(r.Crumbs) == 0 {
		return ""
	}
	revCrumbs := make([]string, len(r.Crumbs))
	for i, s := range r.Crumbs {
		revCrumbs[len(r.Crumbs)-1-i] = s
	}
	return "/" + strings.Join(revCrumbs, "/")
}

func addCrumb(result *Result, crumb string) {
